			...
			using (var nodeBuilder = NodeBuilderEx.Create())
			{
				...
				foreach (var anyoneCanPay in new[] { false, true })
				{
					...
					foreach (var hashType in new[] { SigHash.All, SigHash.Default, SigHash.None, SigHash.Single })
					{
						...
						var hash = spender.GetSignatureHashSchnorr(new TaprootHashContext(spender, new[] { spentOutput.TxOut }), 0,
																	sighash,
																	HashVersion.Taproot, null);
						var sig = key.SignBIP340(hash, sighash);
						Assert.True(addr.PubKey.VerifyBIP340(hash, sig.SchnorrSignature));
