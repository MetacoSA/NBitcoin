...
#nullable enable
	public class UnblindedSignature
	{
		...
		public static bool TryParse(ReadOnlySpan<byte> in64, out UnblindedSignature? unblindedSignature)
		{
			...
			if (in64.Length != 64)
				return false;
			var c = new Secp256k1.Scalar(in64.Slice(0, 32), out overflow);
			if (c.IsZero || overflow != 0)
				return false;
			var s = new Secp256k1.Scalar(in64.Slice(32, 32), out overflow);
		...
		public override string ToString()
		{
			...
			ToBytes(tmp);
			return DataEncoders.Encoders.Hex.EncodeData(tmp);
		}
	}
#nullable restore
#else
