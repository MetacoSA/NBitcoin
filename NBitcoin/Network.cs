...
namespace NBitcoin
{
	...
	public class Network
	{
		...
		static Network()
		{
            _StratisMain = new Network();
            _StratisMain.InitStratisMain();
            _StratisMain.Consensus.Freeze();
        ...
        private void InitStratisMain()
        {
            // for the purpose of easy merging from NBitcoin the stratis network will have its own initialize method
            // while NBitcoin is still in development its easier this way 
            // it will also make it easy to know wich of the tests are targeting stratis.
            // when NStratis becomes an standalone project all Bitcoin network references will be converted to stratis only
            ...
            genesis = CreateStratisGenesisBlock(1470467000, 1831645, 0x1e0fffff, 1, Money.Zero);
        ...
        private Block CreateStratisGenesisBlock(uint nTime, uint nNonce, uint nBits, int nVersion, Money genesisReward)
        {
            ...
            return CreateStratisGenesisBlock(pszTimestamp, genesisOutputScript, nTime, nNonce, nBits, nVersion, genesisReward);
        }

        private Block CreateStratisGenesisBlock(string pszTimestamp, Script genesisOutputScript, uint nTime, uint nNonce, uint nBits, int nVersion, Money genesisReward)
        {
