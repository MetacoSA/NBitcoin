	...
	public class Network
	{
		...
		public static T Parse<T>(string str, Network expectedNetwork = null) where T : IBitcoinString
		{
			...
			if(maybeb58)
			{
				...
				if(maybeb58)
				{
					foreach(var candidate in GetCandidates(str))
					{
		...
		private static IEnumerable<IBase58Data> GetCandidates(string base58)
		{
			if(base58 == null)
		...
		internal static string CreateBase58(Base58Type type, byte[] bytes, Network network)
		{
			...
			var versionBytes = network.GetVersionBytes(type);
