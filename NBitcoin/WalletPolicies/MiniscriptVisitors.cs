...
namespace NBitcoin.WalletPolicies
{
	...
	internal class GenerateScriptVisitor : MiniscriptVisitor
	{
		Stack<List<Op>> ops = new();
		internal Script ToScript(MiniscriptNode rootNode)
		{
			Visit(rootNode);

			if (ops.Count is 0)
				return Script.Empty;
			return new Script(ops.Pop());
		}
	...
	internal class ReadableScriptVisitor(Network Network, KeyType KeyType) : MiniscriptRewriterVisitor
	{
		...
		internal string GenerateScript(MiniscriptNode rootNode)
		{
			...
			var script = new GenerateScriptVisitor().ToScript(frag);
