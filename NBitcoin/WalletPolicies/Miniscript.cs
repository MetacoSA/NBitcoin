...
namespace NBitcoin.WalletPolicies
{
	...
	public class Miniscript
	{
		...
		private static bool TryParseNamedParameter(ParsingContext ctx, ParameterRequirement requirement, [MaybeNullWhen(false)] out MiniscriptNode node)
		{
			...
			node = null;
			if (!ctx.ParsingSettings.AllowedParameters.HasFlag(ParameterTypeFlags.NamedParameter))
				return false;
			var key = Regex.Match(ctx.Remaining, @"^[a-zA-Z0-9_]+");
		...
		private static bool TryParseKey(ParsingContext ctx, [MaybeNullWhen(true)] out MiniscriptError error, [MaybeNullWhen(false)] out MiniscriptNode node)
		{
			node = null;
			error = null;
			...
			if (MultipathNode.TryParse(ctx, out node) ||
		...
		public TaprootInfo? GetTaprootInfo()
		{
			if (RootNode is not TaprootNode tn)
				return null;
			var merkleRoot = GetMerkleRootVisitor.GetMerkleRoot(tn);
