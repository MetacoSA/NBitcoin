...
namespace NBitcoin.WalletPolicies
{
	...
	public class MiniscriptParsingSettings
	{
		...
		/// <summary>
		/// Whether key placeholders or named parameters are allowed.
		/// </summary>
		public ParameterTypeFlags AllowedParameters { get; set; } = ParameterTypeFlags.All;
	...
	public class Miniscript
	{
		...
		Miniscript(MiniscriptNode rootNode, Network network, KeyType keyType)
		{
			if (network is null)
		...
		class Context
		{
			...
			public Context(string miniscript, MiniscriptParsingSettings settings)
			{
		...
		private static bool TryParseBytes(Context ctx, int requiredBytes, [MaybeNullWhen(true)] out MiniscriptError error, [MaybeNullWhen(false)] out MiniscriptNode node)
		{
			...
			if (CanParseName(ctx) && TryParseName(ctx.Remaining, out var name))
			{
		...
		private static bool CanParseName(Context ctx) => ctx.ParsingSettings.AllowedParameters.HasFlag(ParameterTypeFlags.NamedParameter);
		private static bool CanParseKeyPlaceholders(Context ctx) => ctx.ParsingSettings.AllowedParameters.HasFlag(ParameterTypeFlags.KeyPlaceholder);
		...
		private static bool TryParseKey(Context ctx, [MaybeNullWhen(true)] out MiniscriptError error, [MaybeNullWhen(false)] out MiniscriptNode node)
		{
			...
			if (key is { Success: true, Length: (33 * 2 or 32 * 2) })
			{
				...
				try
				{
					...
					{
						(33 * 2, null or KeyType.Classic) => new Value.PubKeyValue(new PubKey(Encoders.Hex.DecodeData(key.Value))),
			...
			if (TryParseMultipathNode(ctx, out var kp))
			{
				node = kp;
				return true;
			}
		...
		private static bool TryParseMultipathNode(Context ctx, [MaybeNullWhen(false)] out MultipathNode kp)
		{
			kp = null;
			var idx = ctx.Remaining.IndexOf("/*");
			if (idx == -1)
				return false;
			var cnt = ctx.Remaining.Length > idx + 2 && ctx.Remaining[idx + 2] == '*' ? 3 : 2;
			var str = ctx.Remaining[0..(idx + cnt)];
			if (!MultipathNode.TryParse(str, ctx.Network, out kp) || (kp.Target is Parameter && !CanParseKeyPlaceholders(ctx)))
				return false;
		...
		{
			"0" or "1" or "pk_k" or "pk_h" or "pk" or "pkh" or "older" or "after" or "sha256" or "ripemd160" or "hash256" or "hash160" or "andor" or "and_v" or "and_b" or "and_n" or "or_b" or "or_c" or "or_d" or "or_i" or "thresh" or "multi" or "multi_a" => true,
		...
		private static bool TryParseMiniscript(Context ctx, [MaybeNullWhen(true)] out MiniscriptError error, [MaybeNullWhen(false)] out Miniscript miniscript)
		{
			...
			{
				var match = Regex.Match(ctx.Remaining, "[a-z]+");
				if (!match.Success)
				{
				...
				using (var frame = ctx.PushFrame())
				{
					...
					else if (descriptor == FragmentDescriptor.tr)
					{
						node = TryParseTaproot(ctx, out error, out var n) ? n : null;
					}
					else if (descriptor == FragmentDescriptor.wsh)
					{
		...
		private static bool TryParseTaproot(Context ctx, [MaybeNullWhen(true)] out MiniscriptError error, [MaybeNullWhen(false)] out MiniscriptNode node)
		{
			error = null;
			node = null;
			if (ctx.IsEnd)
		...
		private static bool TryParseTaprootTree(Context ctx, [MaybeNullWhen(true)] out MiniscriptError error, [MaybeNullWhen(false)] out MiniscriptNode node)
		{
			error = null;
			node = null;
			ctx.SkipSpaces();
			...
			if (ctx.NextChar == '{')
			{
				...
				ctx.Advance(1);
				if (!TryParseTaprootTree(ctx, out error, out var right))
					return false;
				ctx.SkipSpaces();
				if (ctx.IsEnd)
				{
		...
		public Scripts ToScripts()
		{
			...
			var scriptPubKey = new GenerateScriptVisitor().ToScript(RootNode);
			Script? redeem = null;
			if (GetScriptCodeNode(RootNode) is { } n)
			{
				redeem = new GenerateScriptVisitor().ToScript(n);;
			}
		...
		public string ToScriptString() => new ReadableScriptVisitor(Network, KeyType).GenerateScript(RootNode);

		/// <summary>
		/// Generate a user-readable way to view the script actually executed (Can be null of taproot)
		/// </summary>
		/// <returns></returns>
		public string? ToScriptCodeString() => GetScriptCodeNode(RootNode) is { } n ? new ReadableScriptVisitor(Network, KeyType).GenerateScript(n) : null;
		...
		public Miniscript Derive(AddressIntent intent, int index)
		...
		public Miniscript[] Derive(DeriveParameters parameters)
		{
			var visitor = new DeriveVisitor(parameters.Intent, parameters.AddressIndexes, KeyType);
			var nodes = visitor.Derive(RootNode);
			Miniscript[] miniscripts = new Miniscript[nodes.Length];
			for (int i = 0; i < miniscripts.Length; i++)
			{
				miniscripts[i] = new Miniscript(nodes[i], Network, KeyType);
			}
			return miniscripts;
		}
		...
		{
			var visitor = new RemoveKeyInformation();
		...
		{
			var visitor = new AddKeyInformation(keys);
