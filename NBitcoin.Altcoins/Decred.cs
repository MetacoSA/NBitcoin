...
namespace NBitcoin.Altcoins
{
	public partial class Decred : NetworkSetBase
	{
		...
		private static readonly Dictionary<ChainName, DecredConsensusFactory> _factories = new();

		internal static DecredConsensusFactory GetFactory(ChainName chainName)
		{
			lock (_factories)
			{
				if (!_factories.TryGetValue(chainName, out var factory))
				{
					factory = new DecredConsensusFactory(chainName);
					_factories[chainName] = factory;
				}
				return factory;
			}
		}
		...
		private static byte[] Blake256(byte[] b, int offset, int length)
		{
			...
			Array.Copy(b, offset, bCopy, 0, length);
		...
		public class DecredConsensus : Consensus
		{
			...
			public override Target GetWorkRequired(ChainedBlock block)
			{
				...
				// Pre-DCP0011 difficulty calculation not yet implemented;
			...
			private uint calcNextBlake3Diff(ChainedBlock prevNode, uint expected)
			{
				...
				var nextDiff = CalcASERTDiff(WorkDiffV2Blake3StartBits, PowLimit.ToBigInteger(),
					(long)TargetTimePerBlock.TotalSeconds, timeDelta, heightDelta, WorkDiffV2HalfLifeSecs);
		...
		public class DecredConsensusFactory : ConsensusFactory
		{
			...
			public override bool SupportsBatchRPC => false;

			public override object[] GetBlockRPCArgs(uint256 blockHash, int verbosity)
			{
				return new object[] { blockHash, true, verbosity == 2 };
			}

			public override object[] GetTxOutRPCArgs(uint256 txid, int index, int tree, bool includeMempool)
			{
				return new object[] { txid, index, tree, includeMempool };
			}

			public override object GetRawTransactionVerboseParam => (object)1;

			public override bool SupportsGetRawTransactionBlockId => false;

			public override object FormatRPCAmount(Money amount)
			{
				return amount.ToUnit(MoneyUnit.BTC);
			}

			public override Money ParseGetBalanceResponse(JToken result)
			{
				// Decred returns {"balances":[{"accountname":"default","spendable":X,...},...]}
				if (result is JObject obj && obj["balances"] is JArray balances)
				{
					decimal total = 0;
					foreach (var account in balances)
						total += account.Value<decimal>("spendable");
					return Money.Coins(total);
				}
				return base.ParseGetBalanceResponse(result);
			}
			...
			public override RPCResponse RPCRequestHook(ref RPCRequest request)
			{
				...
				{
					case "getnewaddress":
						// Decred does not support named params for getnewaddress.
						request.NamedParams = null;
						request.Params = Array.Empty<object>();
						break;

					case "getbalance":
						// Decred does not support includeWatchOnly param.
						if (request.Params.Length > 2)
							request.Params = request.Params.Take(2).ToArray();
						break;

					case "fundrawtransaction":
						// Decred requires an account name after the tx hex.
						if (request.Params.Length >= 1)
						{
							var ftParams = request.Params.ToList();
							if (ftParams.Count < 2 || !(ftParams[1] is string))
								ftParams.Insert(1, "default");
							request.Params = ftParams.ToArray();
						}
						break;
					...
					case "sendmany":
						...
						foreach (var property in addressAmounts)
						{
							if (property.Value.Type == JTokenType.String)
								keysToConvert.Add(property.Key);
						}
						foreach (var key in keysToConvert)
						{
							var amount = decimal.Parse(addressAmounts[key].Value<string>());
							addressAmounts[key] = amount;
						}
						break;
				}
			...
			public override bool ParseGetBlockRPCResponse(JObject json, bool withFullTx, out BlockHeader blockHeader, out Block block, out List<uint256> txids)
			{
				...
				decredBH.BlockSize = json.Value<uint>("size");
				...
				decredBH.SBits = (long)(json.Value<decimal>("sbits") * 100_000_000m);
				...
				else
				{
					...
					foreach (var stx in json.Value<JArray>("stx") ?? [])
					{
						txids.Add(uint256.Parse(stx.ToString()));
					}
				}
			...
			{
				if (command == "initstate")
					return new DecredInitStatePayload();
				return base.CreatePayload(command);
			}
		...
		public class DecredTxIn : TxIn
		{
			...
			{
				return Decred.GetFactory(chainName);
			}
		}
		...
		public class DecredTransaction : Transaction
		{
			...
			// Decred sighash serialization type constants.
			const ushort SigHashSerializePrefix = 1;
			const ushort SigHashSerializeWitness = 3;
			...
			{
				var baseHashType = (SigHash)((uint)nHashType & 0x1f);
				var anyoneCanPay = ((uint)nHashType & (uint)SigHash.AnyoneCanPay) != 0;

				// Prefix hash.
				var hs = this.CreateHashStream();
				var stream = new BitcoinStream(hs, true);
				this.serializeSigHashPrefix(stream, nIn, baseHashType, anyoneCanPay);
				var prefixHash = hs.GetHash();

				// Witness hash.
				hs = this.CreateHashStream();
				stream = new BitcoinStream(hs, true);
				this.serializeSigHashWitness(stream, scriptCode, nIn, anyoneCanPay);
				var witnessHash = hs.GetHash();

				// Final hash: BLAKE256(hashType || prefixHash || witnessHash).
				hs = this.CreateHashStream();
				stream = new BitcoinStream(hs, true);
				var hashTypeB = BitConverter.GetBytes((uint)nHashType);
			...
			private void serializeSigHashPrefix(BitcoinStream stream, int nIn, SigHash baseHashType, bool anyoneCanPay)
			{
				// Version with SigHashSerializePrefix type.
				ushort version = (ushort)this.Version, sType = SigHashSerializePrefix;
				stream.ReadWrite(ref version);
				stream.ReadWrite(ref sType);

				// Determine which inputs to include.
				var txIns = this.Inputs.Select((input, i) => (input: (DecredTxIn)input, index: i));
				int signInputIdx = nIn;
				if (anyoneCanPay)
				{
					txIns = txIns.Where(x => x.index == nIn);
					signInputIdx = 0;
				}
				var txInList = txIns.ToList();

				// Inputs.
				var txInCount = (uint)txInList.Count;
				stream.ReadWriteAsVarInt(ref txInCount);
				for (int i = 0; i < txInList.Count; i++)
				{
					var input = txInList[i].input;
					stream.ReadWrite(input.PrevOut);
					stream.ReadWriteBytes([input.PrevOutTree]);

					// For SigHashNone and SigHashSingle, sequences of
					// non-signing inputs are set to 0.
					uint sequence = input.Sequence;
					if (i != signInputIdx && (baseHashType == SigHash.None || baseHashType == SigHash.Single))
						sequence = 0;
					stream.ReadWrite(ref sequence);
				}
				...
				switch (baseHashType)
				{
					case SigHash.None:
						// No outputs.
						uint zero = 0;
						stream.ReadWriteAsVarInt(ref zero);
						break;

					case SigHash.Single:
						// Outputs 0..nIn. Pre-nIn outputs get value=-1 and empty script.
						var singleCount = (uint)(nIn + 1);
						stream.ReadWriteAsVarInt(ref singleCount);
						for (int i = 0; i < singleCount; i++)
						{
							if (i < nIn)
							{
								ulong negOne = unchecked((ulong)-1L);
								stream.ReadWrite(ref negOne);
								ushort outVersion = (ushort)((DecredTxOut)this.Outputs[i]).Version;
								stream.ReadWrite(ref outVersion);
								var emptyScript = new Script();
								stream.ReadWrite(ref emptyScript);
							}
							else
							{
								var output = (DecredTxOut)this.Outputs[i];
								stream.ReadWrite(output.Value);
								ushort outVersion = (ushort)output.Version;
								stream.ReadWrite(ref outVersion);
				...
				stream.ReadWrite(this.LockTime.Value);
				stream.ReadWrite(this.Expiry);
			}

			private void serializeSigHashWitness(BitcoinStream stream, Script scriptCode, int nIn, bool anyoneCanPay)
			{
				// Version with SigHashSerializeWitness type.
				ushort version = (ushort)this.Version, sType = SigHashSerializeWitness;
				stream.ReadWrite(ref version);
				stream.ReadWrite(ref sType);

				// Determine which inputs to include.
				int inputCount = anyoneCanPay ? 1 : this.Inputs.Count;
				int signInputIdx = anyoneCanPay ? 0 : nIn;

				var txInCount = (uint)inputCount;
				stream.ReadWriteAsVarInt(ref txInCount);
				for (int i = 0; i < inputCount; i++)
				{
					// Only the signing input gets the actual script.
					var script = new Script();
					if (i == signInputIdx)
						script = scriptCode;
					stream.ReadWrite(ref script);
...
#pragma warning disable CS0618 // Type or member is obsolete
		public class DecredBlock : Block
		{
			...
			public override ConsensusFactory GetConsensusFactory()
			{
				return Decred.GetFactory((Header as DecredBlockHeader).ChainName);
			}
		}
		...
		public class DecredBase58CheckEncoder : Base58CheckEncoder
		{
			...
			{
				// Decred WIF: [version(2)][sigScheme(1)][key(32)]
				// NBitcoin gives us: [version(2)][key(32)][compression(1)]
				if (count >= 35 && isBase58Type(data, offset, Base58Type.SECRET_KEY))
				{
					var transformed = new byte[count];
					Buffer.BlockCopy(data, offset, transformed, 0, 2); // version
					transformed[2] = 0x00; // STEcdsaSecp256k1 sig scheme
					Buffer.BlockCopy(data, offset + 2, transformed, 3, 32); // key (drop compression byte)
					return base.EncodeData(transformed, 0, transformed.Length);
				}
				return base.EncodeData(data, offset, count);
			}

			public override byte[] DecodeData(string encoded)
			{
				var data = base.DecodeData(encoded);
				// Decred WIF decoded: [version(2)][sigScheme(1)][key(32)] = 35 bytes
				// NBitcoin expects: [version(2)][key(32)][compression(1)] = 35 bytes
				if (data.Length == 35 && isBase58Type(data, 0, Base58Type.SECRET_KEY))
				{
					var result = new byte[35];
					Buffer.BlockCopy(data, 0, result, 0, 2); // version
					Buffer.BlockCopy(data, 3, result, 2, 32); // key (skip sig scheme)
					result[34] = 0x01; // compressed
					return result;
				}
				return data;
			}
			...
			{
				return isBase58Type(bytes, 0, base58Type);
			}

			protected bool isBase58Type(byte[] bytes, int offset, Base58Type base58Type)
			{
		...
		public class DecredAddressStringParser : NetworkStringParser
		{
			...
			public override BitcoinPubKeyAddress CreateP2PKH(PubKey pubKey, Network network)
			{
				var keyId = new KeyId(_hash160(pubKey.ToBytes()));
				return CreateP2PKH(keyId, network);
			}
		}

		private static uint160 _hash160(byte[] data)
		{
			return _hash160(data, 0, data.Length);
		}
		...
		protected override NetworkBuilder CreateMainnet()
		{
			...
			.SetConsensus(new DecredConsensus()
			{
				...
				ConsensusFactory = new DecredConsensusFactory(ChainName.Mainnet) { WalletRPCPort = 9110 },
		...
		protected override NetworkBuilder CreateRegtest()
		{
			...
			.SetConsensus(new DecredConsensus()
			{
				...
				ConsensusFactory = new DecredConsensusFactory(ChainName.Regtest) { WalletRPCPort = 19562 },
				CoinbaseMaturity = 16,
		...
		protected override NetworkBuilder CreateTestnet()
		{
			...
			.SetConsensus(new DecredConsensus()
			{
				...
				ConsensusFactory = new DecredConsensusFactory(ChainName.Testnet) { WalletRPCPort = 19110 },
				CoinbaseMaturity = 16,
		...
		public class DecredInitStatePayload : Payload, IBitcoinSerializable
		{
			...
			private ulong blockHashCount = 0;
			private ulong voteHashCount = 0;
			private ulong tSpendHashCount = 0;

			public DecredInitStatePayload() { }

			public override void ReadWriteCore(BitcoinStream stream)
			{
				stream.ReadWriteAsVarInt(ref blockHashCount);
				stream.ReadWriteAsVarInt(ref voteHashCount);
				stream.ReadWriteAsVarInt(ref tSpendHashCount);
			}
		}
