...
#pragma warning disable CS0618 // Type or member is obsolete
		public class DystemBlockHeader : BlockHeader
		{
			...
			static byte[] CalculateHash(byte[] data, int offset, int count)
			{
					...
					var h = new HashQuark.Quark().ComputeBytes(data.Skip(offset).Take(count).ToArray());
				
				//var tx=Encoders.Hex.EncodeData(data.Skip(offset).Take(count).ToArray());

				//var htx=Encoders.Hex.EncodeData(h);
